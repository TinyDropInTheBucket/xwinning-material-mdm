...
public class ManufacturerContactEntity extends BaseEntity{
    ...
    @OneToMany(targetEntity = OrganizationContactNameSetEntity.class, cascade = CascadeType.MERGE, fetch = FetchType.EAGER)
    @JoinColumn(name = "ORG_CONTACT_ID", referencedColumnName = "ORG_CONTACT_ID")
    private List<OrganizationContactNameSetEntity> contactNameList;
